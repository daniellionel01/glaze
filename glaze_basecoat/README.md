...
| Use Tailwind CDN | Tailwind CDN + Basecoat CSS via CDN | All Basecoat JS via CDN | Tailwind `<script ...@tailwindcss/browser@4>` + `glaze_basecoat.register(version)` + `theme.style_tag(...)` + `theme.tailwind_v4_bridge_style_tag()` |
| Build-time Tailwind (Tailwind CLI/PostCSS/Vite) | Your Tailwind build output (with `@import "basecoat-css"`) | Basecoat JS via CDN | `glaze_basecoat.register_js(version)` (or `register_component(version, "tabs")`) + `theme.style_tag(...)` |
